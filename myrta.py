...
try:
    ...
    web_browser_name = str.lower(config.get("system", "web_browser_name"))
    ...
    weekdays_vec = str.split(weekday_tri, ',')
#     pp.pprint(weekdays_vec)
    log_file = os.path.join(root_dir, config.get("system", "log_file"))
    print(log_file)
    log_level = config.get("system", "log_level")
    logging.basicConfig(filename=log_file, filemode='w', level=logging.getLevelName(log_level), format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    ...
    try:
        ...
        while len(tasks) > 0:
            ...
            for task_id in range(tasks_count):
                ...
                #Step 1: View Booking
                chgtime = driver.find_element_by_css_selector("[widgetid=changeTimeButton]")
                chgtime.click()
                time.sleep(action_wait_time)
        
                #Step 3: Change Date / Time
                year = str.strip(task_items[2])
                month = str.strip(task_items[3])
                day = str.strip(task_items[4])
                timeslot = str.strip(task_items[5])
